...
def get_weather_info(test_pd, airport_name='ATL', lamp=None):
    """
    ...
    final_pd = pd.merge_asof(test_pd.sort_values(by='timestamp'), weather_pd.sort_values(by='timestamp'),
    ...
    numeric_features = ['wind_speed', 'wind_direction', 'temperature', 'wind_gust', 'cloud_ceiling', 'visibility']
...
def get_meta_features(test_pd, mfs):
    ...
    """
    meta_cat_cols = ['aircraft_engine_class', 'aircraft_type', 'major_carrier', 'flight_type']
...
def get_dep_rw(test_pd, runways):
    ...
    final_pd = pd.merge_asof(test_pd.sort_values(by='timestamp'), rw.sort_values(by='timestamp'), on='timestamp',
                             by='gufi', tolerance=pd.Timedelta("30h"), direction='backward')

    final_pd['departure_runway_actual'].fillna('NA', inplace=True)
    final_pd['departure_runway_actual'] = final_pd['departure_runway_actual'].astype('category')
...
airports_list = ["KATL", "KCLT", "KDEN", "KDFW", "KJFK", "KMEM", "KMIA", "KORD", "KPHX", "KSEA"]
...
def load_model(solution_directory: Path) -> Any:
    ...
    for airport_name in airports_list:
        ...
        model[airport_name].load_model(solution_directory /'v5_model_{}_{}.json'.format(airport_name, lr ))
    return model
...
def predict(
    ...
    features = []
    cat_features = []
    USED_COLS =  [x for x in partial_submission_format.columns.tolist() if x not in ['minutes_until_pushback']]
    ...
    # lamp features
    submission_pd, fts_dict = get_weather_info(submission_pd,lamp=lamp)
    features += fts_dict['numeric'] + fts_dict['cat']
    cat_features += fts_dict['cat']
    # basic features
    ...
    prediction = pd.merge(prediction[USED_COLS],  submission_pd[['gufi','timestamp', 'minutes_until_pushback']] , on =['gufi','timestamp'], how ='left')

    #submission_pd['minutes_until_pushback'] = y_pred
    prediction['minutes_until_pushback'] = prediction['minutes_until_pushback'].apply(lambda x: max(1, int(x)))
